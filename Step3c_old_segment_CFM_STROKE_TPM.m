%% UNIFIED SEGMENTATION WITH CFM AND TPM FROM THE CLINICAL TOOLBOX
...
ses='ses-001';
anat='anat';
prep_folder='spm_us_cfm_cleanup_STROKE_TPM_med_reg_old_NORM'; % Write your preprocessing folder name here
...
for sub=1:size(names,1)
    ...
    if ~strcmp(names(sub).name, 'sub-24' )  && ~strcmp(names(sub).name, 'sub-31' ) && ~strcmp(names(sub).name, 'sub-32')  && ~strcmp(names(sub).name, 'sub-33') && ~strcmp(names(sub).name, 'sub-35')
        ...
        copyfile(fullfile(sub_path1,T1(2,:)),fullfile(sub_path1,prep_folder))
        copyfile(fullfile(sub_path1,rinv_lesion),fullfile(sub_path1,prep_folder))
        matlabbatch{1}.spm.tools.oldseg.data ={ deblank(fullfile(sub_path1,prep_folder,T1(2,:)))};
        ...
        matlabbatch{1}.spm.tools.oldseg.opts.tpm = {
            'C:\Program Files\spm12\toolbox\OldSeg\scgrey.nii' %Grey TPM from the clinical toolbox
            'C:\Program Files\spm12\toolbox\OldSeg\scwhite.nii' %White TPM from the clinical toolbox
            'C:\Program Files\spm12\toolbox\OldSeg\sccsf.nii' %CSF TPM from the clinical toolbox
            };
        
        %This TPMs are derived from  50 individuals with a mean age of 72.9 years old (median = 74; stdev = 7.63), with 18 men.
        %Ref: Bonilha, L., Eckert, M. A., Fridriksson, J., Hirth, V. A., Moser, D., Morgan, P. S., & Rorden, C. (2009). Age-related relative volume preservation of the dominant hand cortical region. Brain research, 1305, 14?19. https://doi.org/10.1016/j.brainres.2009.10.001 
